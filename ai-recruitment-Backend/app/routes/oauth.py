...
async def microsoft_login():
    ...
    return RedirectResponse(url=f"https://login.microsoftonline.com/common/oauth2/v2.0/authorize?{params}")


@router.get("/microsoft/callback")
async def microsoft_callback(code: str = None, error: str = None, db: AsyncSession = Depends(get_db)):
    if error or not code:
        return RedirectResponse(url=f"{settings.frontend_url}/login?error=oauth_failed")
    try:
        redirect_uri = f"{settings.backend_url}/api/v1/auth/oauth/microsoft/callback"
        ...
        import logging
        logging.error(f"Microsoft OAuth error: {e}")
        return RedirectResponse(url=f"{settings.frontend_url}/login?error=oauth_failed")
