...
export class WebRTCManager {
  ...
  createPeerConnection(socketId, userName, role) {
    ...
    // Add local tracks — CRITICAL: do NOT call addTransceiver after addTrack
    // Calling both causes duplicate transceivers which breaks negotiation
    if (this.localStream) {
    ...
    // Remote stream — this is the handler that delivers video+audio to the UI
    pc.ontrack = (event) => {
      console.log('[WebRTC] ontrack fired — kind:', event.track.kind, 'streams:', event.streams.length);
      // Always use event.streams[0] when available (contains both audio+video)
  ...
  async createOffer(socketId, userName, role) {
    ...
    try {
      ...
      console.log('[WebRTC] Offer created and local description set, sending to', socketId);
  ...
  async handleOffer(socketId, offer, userName, role) {
    ...
    try {
      ...
      console.log('[WebRTC] Answer created and local description set, sending to', socketId);
